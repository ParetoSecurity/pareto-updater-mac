...
public class AppUpdater: Hashable, Identifiable {
    ...
    var help: String {
        "Installed: \(latestVersion), latest: \(currentVersion)"
    }
    ...
    func downloadLatest(completion: @escaping (URL, URL) -> Void) {
        ...
        AF.download(latestURL).responseData { [self] response in
            ...
            os_log("Progress of %{public}s is %{public}f %", latestURL.debugDescription, progress.fractionCompleted)
        }
    }
